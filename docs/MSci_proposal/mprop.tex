...
\par Throughout the literature review, we have looked into the papers which are considered to have a significant influence on the LDA-like modelling. For this reason, we have set up the basis of the LDA-like model mathematical underlying. However, the approach in utilising those models in MSI like data sets have not been reviewed. Nevertheless, two sources of relevant research have been mentioned (the paper by Hooft et al. \cite{hooft} and the survey by Alonso et al. \cite{alonso-et-al}). Ultimately, this section is focused to emphasise the superior approaches of the previously reviewed papers. 
...
\end{figure}
In the boundary case shown on the left, time-slice $t_{0,0}$ relates to  three surrounding time-slices, whereas, in the general case indicated on the right side, time-slice $t_{1,1}$ relates to eight surrounding time-slices. 


time-slice $t$ would be dependant on it's adjacent time-slices as shown  
Even though the dynamic topic modelling suggests 
...
% -- Dictionary
\par
% To-do: induce time-series in terms of z axis 
